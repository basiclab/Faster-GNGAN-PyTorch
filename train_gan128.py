...
from source.losses import HingeLoss, BCEWithLogits
...
class Trainer:
    def __init__(self):
        ...
        if FLAGS.resume:
            ...
            self.writer.flush()
            # start value
            self.start, self.best_IS, self.best_FID = 0, 0, 999
    ...
    def train(self):
        with trange(self.start, FLAGS.total_steps, dynamic_ncols=True,
    ...
    def train_step(self, x):
        ...
        for _ in range(FLAGS.n_dis):
            ...
            for _ in range(FLAGS.D_accumulation):
                ...
                pred_real, pred_fake = self.net_GD(z[: FLAGS.batch_size], real)
